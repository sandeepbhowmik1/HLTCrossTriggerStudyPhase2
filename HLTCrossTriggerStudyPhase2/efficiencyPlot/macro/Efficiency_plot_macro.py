...
class EfficiencyPlot:
    ...
    def plot(self):
        ...
        extraTextBox2 = ROOT.TLatex  (self.xposText, self.yposText - 0.04, self.extraText2)
        extraTextBox2.SetNDC()
        extraTextBox2.SetTextSize(extraTextSize)

        extraTextBox3 = ROOT.TLatex  (self.xposText, self.yposText - 0.08, self.extraText3)
        extraTextBox3.SetNDC()
        extraTextBox3.SetTextSize(extraTextSize)

        extraTextBox4 = ROOT.TLatex  (self.xposText, self.yposText - 0.12, self.extraText4)
        extraTextBox4.SetNDC()
        extraTextBox4.SetTextSize(extraTextSize)

        extraTextBox5 = ROOT.TLatex  (self.xposText, self.yposText - 0.16, self.extraText5)
        extraTextBox5.SetNDC()
        extraTextBox5.SetTextSize(extraTextSize)

        extraTextBox6 = ROOT.TLatex  (self.xposText, self.yposText - 0.20, self.extraText6)
        extraTextBox6.SetNDC()
        extraTextBox6.SetTextSize(extraTextSize)
	...
	legend.SetTextSize(extraTextSize)
        ...
        for turnon in self.turnons:
            ...
            #if(turnon.legend == "None"):
            #if(turnon.markerStyle == 24):
                #continue
            legend.Draw()
        CMSbox.Draw()
        lumibox.Draw()
